...
export default function TeamPage() {
  ...
  const loadData = async (isRefresh = false) => {
    ...
    try {
      // CANONICAL: Load from Membership only
      const membersData = await base44.entities.Membership.filter({ tenant_id: tenantId });
      
      // Use user_email from Membership - no need to query User entity
      // This avoids ACL issues and ensures workspace isolation
      setMembers(membersData);
      setUsers([]); // Not needed - we use user_email from membership
    } catch (error) {
      toast({
